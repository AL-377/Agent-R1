...
class MedDialogCNExaminer(BaseExaminerAgent):
    ...
    def normalize_dialogue(self, raw_record: Any) -> Tuple[str, List[Dict[str, str]], Dict[str, Any]]:
        ...
        Expected formats:
          Format A (HF medical_dialog):
            {"utterances": ["患者：...", "医生：...", ...]}
          Format B (alternative):
            {"description": "...", "dialogue": [{"speaker": "patient", "text": "..."}]}
          Format C (simple):
            {"utterances": ["text1", "text2", ...]}  (alternating patient/doctor)
        """
        # Generate unique ID
        ...
        # Extract description if present (patient condition/history)
