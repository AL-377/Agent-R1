...
class CMtMedQAExaminer(BaseExaminerAgent):
    ...
    def normalize_dialogue(self, raw_record: Any) -> Tuple[str, List[Dict[str, str]], Dict[str, Any]]:
        ...
        Supported formats:
          Format A (HuggingFace/Suprit):
            {"id": "j0", "cate1": "男科", "cate2": "前列腺科",
             "instruction": "...", "input": "", "output": "...",
             "history": [["患者问", "医生答"], ...]}
          Format B (legacy):
            {"question": "...", "answer": "...", "department": "...", "title": "..."}
          Format C (pre-structured):
        ...
        # Extract metadata from various field names
        for key in ["department", "title", "category", "科室", "cate1", "cate2"]:
        ...
        # Format A: instruction/output/history (Suprit/CMtMedQA on HuggingFace)
        history = raw_record.get("history", [])
        instruction = raw_record.get("instruction", "").strip()
        output = raw_record.get("output", "").strip()

        if history or instruction:
            # Replay the conversation history first
            if isinstance(history, list):
                for turn_pair in history:
                    if isinstance(turn_pair, (list, tuple)) and len(turn_pair) >= 2:
                        patient_msg = str(turn_pair[0]).strip()
                        doctor_msg = str(turn_pair[1]).strip()
                        if patient_msg:
                            messages.append({"role": "patient", "content": patient_msg})
                        if doctor_msg:
                            messages.append({"role": "doctor", "content": doctor_msg})

            # Then add the current instruction (patient) and output (doctor)
            if instruction:
                messages.append({"role": "patient", "content": instruction})
            if output:
                messages.append({"role": "doctor", "content": output})

            if messages:
                return dialogue_id, messages, metadata

        # Format B: single Q&A with question/answer fields
    ...
    def grouping_key_fn(self, normalised_record: Dict[str, Any]) -> str:
        ...
        # Priority: cate1 > cate2 > department > category > title keyword
        for key in ["cate1", "cate2", "department", "科室"]:
            val = meta.get(key, "")
            if val and str(val).strip() and str(val).strip().lower() != "nan":
